...
//    }
    ...
    // check is local file
    NSURL *fullURL = [[AppManager sharedManager] fetchLocalVideoURL:path.lastPathComponent];
    if (fullURL == nil)
    {
        // stream url
        NSString *p = [path stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        fullURL = [NSURL URLWithString:p];
    }
    // create the asset
    AVAsset *asset = [AVAsset assetWithURL:fullURL];
