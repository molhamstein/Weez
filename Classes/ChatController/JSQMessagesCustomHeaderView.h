...
@class JSQMessagesCustomHeaderView;

/**
 *  A constant defining the default height of a `JSQMessagesLoadEarlierHeaderView`.
 */
FOUNDATION_EXPORT const CGFloat kJSQMessagesCustomHeaderViewHeight;
 ...
 */
@protocol JSQMessagesCustomHeaderViewDelegate <NSObject>
 ...
 */
- (void)headerView:(JSQMessagesCustomHeaderView *)headerView didPressLoadButton:(UIButton *)sender;
 ...
 */
@interface JSQMessagesCustomHeaderView : UICollectionReusableView

/**
 *  The object that acts as the delegate of the header view.
 */
@property (weak, nonatomic) id<JSQMessagesCustomHeaderViewDelegate> delegate;
